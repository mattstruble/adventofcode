...
2-6,4-8"""
    ...
    def puzzle_one(self, data: list[str]) -> int:
        ...
        for line in data:
            ...
            if (
                first_assignment[0] <= second_assignment[0]
                and first_assignment[1] >= second_assignment[1]
            ):
                result += 1
            elif (
                second_assignment[0] <= first_assignment[0]
                and second_assignment[1] >= first_assignment[1]
            ):
                result += 1
        return result
    ...
    def puzzle_two(self, data: list[str]) -> int:
        ...
        for line in data:
            first_assignment, second_assignment = get_assignments(line)
            if (
                first_assignment[0] <= second_assignment[1]
                and first_assignment[0] >= second_assignment[0]
            ) or (
                second_assignment[0] <= first_assignment[1]
                and second_assignment[0] >= first_assignment[0]
            ):
                result += 1
