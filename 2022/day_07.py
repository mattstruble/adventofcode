...
class Node: 
    def __init__(self, name, *, value=None, children:dict[str, "Node"]=None, parent:"Node"=None) -> None:
        self.name = name 
        self.parent = parent 
    ...
    def add_child(self, name, value=None, children=None): 
        self.children[name] = Node(name=name, value=value, children=children, parent=self)
...
7214296 k"""
    ...
    def puzzle_one(self, data: list[str]) -> int:
        ...
        return sum([node.sum_values() for node in node_iterator(root) if node.has_children() and node.sum_values() < 100_000])
    ...
    def puzzle_two(self, data: list[str]) -> int:
        ...
        for node in node_iterator(root):
            ...
            if node.has_children() and node_size > needed_space and node_size < smallest_dir_size:
